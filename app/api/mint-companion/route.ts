...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch RPC URL from environment variables via API
    const rpcResponse = await fetch(`${request.nextUrl.origin}/api/env?key=RPC_API_URL`);
    const rpcData = await rpcResponse.json();
    const rpcUrl = /*rpcData.value || */ 'https://api.devnet.solana.com';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fetch RPC URL from environment variables via API
    const rpcResponse = await fetch(`${request.headers.get('origin')}/api/env?key=RPC_API_URL`);
    const rpcData = await rpcResponse.json();
    const rpcUrl = /*rpcData.value || */ 'https://api.devnet.solana.com';
